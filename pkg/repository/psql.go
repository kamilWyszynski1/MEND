...
const (
	insertUserQuery = "INSERT INTO user(name, surname) VALUES ($1, $2, $3)"
	getUserQuery    = "SELECT FROM user WHERE id = $1"
	updateUserQuery = "UPDATE user SET name = $1, surname = $2 WHERE id = $3"
...
func (p *PSQL) Create(ctx context.Context, user models.User) (int64, error) {
	result, err := p.client.ExecContext(ctx, insertUserQuery, user.ID, user.Name, user.Surname)
	if err != nil {
...
func (p *PSQL) Update(ctx context.Context, user models.User) error {
	_, err := p.client.ExecContext(ctx, updateUserQuery, user.Name, user.Surname, user.ID)
	if err != nil {
